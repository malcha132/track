...
<body class="min-h-screen text-gray-100 p-2 md:p-6">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
            <div class="bg-gradient-to-br from-green-600/20 to-green-700/20 border border-green-500/30 rounded-lg p-4 shadow-lg">
                <div class="text-green-400 text-xs uppercase tracking-wider mb-1 font-semibold">Celkový Profit</div>
                <div id="totalProfit" class="text-2xl font-bold">0 Kč</div>
            </div>
            ...
            <div class="bg-gray-800/50 border border-gray-700 rounded-lg p-4 shadow-lg">
                <div class="text-gray-400 text-xs uppercase tracking-wider mb-1 font-semibold">Hodinový výdělek</div>
                <div id="hourlyRate" class="text-2xl font-bold text-gray-100">0 Kč/h</div>
            </div>
        </div>

        <div class="bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-4 mb-6">
            <h2 class="text-lg font-bold mb-4 text-green-400 flex items-center">➕ <span class="ml-2">Nová session</span></h2>
            <form id="sessionForm" class="grid grid-cols-2 md:grid-cols-6 gap-3 items-end">
                ...
                <div>
                    ...
                    <input type="number" id="buyin" required min="0" class="w-full px-3 py-1.5 bg-gray-900 border border-gray-600 rounded text-sm focus:ring-1 focus:ring-green-500 outline-none" placeholder="Kč">
                </div>
                <div>
                    <label class="block text-xs font-medium mb-1 text-gray-400">Cashout</label>
                    <input type="number" id="cashout" required min="0" class="w-full px-3 py-1.5 bg-gray-900 border border-gray-600 rounded text-sm focus:ring-1 focus:ring-green-500 outline-none" placeholder="Kč">
                </div>
                <div>
                    <label class="block text-xs font-medium mb-1 text-gray-400">Čas (h)</label>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-lg shadow-xl overflow-hidden mb-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full table-xs">
                    <thead class="bg-gray-900/50 text-gray-400 border-b border-gray-700">
                        <tr>
                            ...
                            <th class="text-right">Hodinová</th>
                ...
                <div id="emptyState" class="text-center py-8 text-gray-600 hidden">
                    <p class="text-sm italic">Žádné záznamy k zobrazení</p>
                </div>
            </div>
        </div>

        <div class="bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-4">
            <h2 class="font-bold text-green-400 uppercase text-sm tracking-widest mb-4 text-center">Vývoj Bankrollu</h2>
    ...
    <script>
                ...
                return `
                    <tr class="hover:bg-gray-700/30 transition-colors">
                        ...
                        <td class="text-center">
                            <button onclick="deleteSession(${s.id})" class="text-gray-500 hover:text-red-500 p-1">✕</button>
                        </td>
                    </tr>
                `;
            }).join('');
        ...
        function updateStatistics() {
            ...
            document.getElementById('totalProfit').textContent = `${profit >= 0 ? '+' : ''}${formatNum(profit)} Kč`;
            ...
            document.getElementById('hourlyRate').textContent = thrs > 0 ? formatNum(profit / thrs) + ' Kč/h' : '0 Kč/h';
        }

        function updateChart() {
            const ctx = document.getElementById('profitChart').getContext('2d');
            
            // Seřazení chronologicky pro graf
        ...
        document.getElementById('clearAll').addEventListener('click', () => {
            if(confirm('Smazat vše?')) { sessions = []; saveAndRender(); }
        });
